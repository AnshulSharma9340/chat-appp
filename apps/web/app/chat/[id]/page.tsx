...
//   if (status === "DELIVERED") {
//     return (
//       <span style={{ fontSize: 13, color: "rgb(250, 242, 242)", letterSpacing: -4 }}>
//         ✓✓
//       </span>
//     );
//   }
...
//   if (status === "SEEN") {
//     return (
//       <span style={{ fontSize: 13, color: "#0560ce", letterSpacing: -4 }}>
//         ✓✓
//       </span>
//     );
//   }
...
//   // KEY FIX: sendMessage ko ref mein rakho
//   // taaki recorder.onstop mein stale closure na ho
//   const sendMessageRef = useRef<(fileOverride?: File) => Promise<void>>(
//     async () => {}
//   );
...
//       subscription = stomp.subscribe(`/topic/room/${roomId}`, (message) => {
...
//         // lowercase mein store karo — consistent rendering ke liye
...
//   // ref hamesha latest sendMessage point kare
//   useEffect(() => {
...
//           if (blob.size === 0) {
//             alert("Recording empty. Please try again.");
//             return;
//           }
...
//           // KEY FIX: ref se call karo — stale closure nahi hogi
...
//   // KEY FIX: backend kabhi uppercase bheje to bhi kaam kare
...
//           <button
//             onClick={leaveRoom}
//             className="px-4 py-1.5 rounded-full text-sm bg-red-500/20 border border-red-400/30 text-red-300 hover:bg-red-500/30 transition"
//           >
//             Leave
//           </button>
//           <button
//             onClick={logout}
//             className="px-4 py-1.5 rounded-full text-sm bg-orange-500/20 border border-orange-400/30 text-orange-300 hover:bg-orange-500/30 transition"
//           >
//             Logout
//           </button>
//         </div>
...
//                         : "bg-white/10 backdrop-blur-sm border border-white/10 rounded-tl-sm text-white/90"
//                     }`}
//                   >
//                     {/* Text */}
//                     {msg.content && (
...
//                     {/* Image — ft() se compare karo */}
//                     {ft(msg) === "image" && msg.fileUrl && (
//                       <img
//                         src={msg.fileUrl}
//                         alt="shared"
//                         className="mt-2 rounded-xl max-w-full max-h-64 object-cover"
//                       />
//                     )}

//                     {/* Video */}
//                     {ft(msg) === "video" && msg.fileUrl && (
//                       <video
//                         controls
//                         className="mt-2 rounded-xl max-w-full max-h-52"
//                       >
...
//                     {/* Audio — custom player */}
//                     {ft(msg) === "audio" && msg.fileUrl && (
//                       <AudioPlayer src={msg.fileUrl} isOwn={isOwn} />
//                     )}

//                     {/* File */}
//                     {ft(msg) === "file" && msg.fileUrl && (
//                       <a
//                         href={msg.fileUrl}
//                         target="_blank"
//                         rel="noopener noreferrer"
//                         className="mt-2 flex items-center gap-2 text-yellow-300 underline underline-offset-2 text-sm"
//                       >
//                         📄 Download File
//                       </a>
//                     )}
...
//               className="px-6 py-2.5 rounded-full bg-indigo-500 hover:bg-indigo-600 text-white font-medium transition"
//             >
//               📸 Capture
//             </button>
//             <button
//               onClick={stopCamera}
//               className="px-6 py-2.5 rounded-full bg-red-500/80 hover:bg-red-600 text-white font-medium transition"
//             >
...
//             {/* Attach */}
//             <label
//               className="cursor-pointer p-2.5 rounded-xl text-white/50 hover:text-white hover:bg-white/10 transition flex-shrink-0"
//               title="Attach file"
//             >
//               <FaPaperclip size={16} />
//               <input
...
//               type="button"
//               onClick={startCamera}
//               className="p-2.5 rounded-xl text-white/50 hover:text-white hover:bg-white/10 transition flex-shrink-0"
//               title="Take photo"
//             >
//               <FaCamera size={16} />
//             </button>
...
//               className="p-2.5 rounded-xl text-white/50 hover:text-white hover:bg-white/10 transition flex-shrink-0 text-base leading-none"
//               title="Emoji"
//             >
//               😊
//             </button>
...
//               className="w-10 h-10 rounded-xl bg-indigo-500 hover:bg-indigo-600 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center transition shadow-lg flex-shrink-0"
//             >
//               {isSending ? (
...
function MessageTick({ status }: { status?: string }) {
  ...
  if (status === "SENT") {
    return (
      <span style={{ fontSize: 13, color: "rgba(255,255,255,0.5)", marginLeft: 2 }}>
        ✓
      </span>
    );
  }
  if (status === "DELIVERED") {
    return (
      <span style={{ fontSize: 13, color: "rgba(255,255,255,0.5)", letterSpacing: -4, marginLeft: 2, paddingRight: 4 }}>
        ✓✓
      </span>
    );
  }
  if (status === "SEEN") {
    return (
      <span style={{ fontSize: 13, color: "#60a5fa", letterSpacing: -4, marginLeft: 2, paddingRight: 4 }}>
        ✓✓
      </span>
    );
  }
...
export default function ChatPage() {
  ...
  // KEY: username ref — taaki WebSocket callbacks mein latest value mile
  ...
  useEffect(() => {
    ...
    if (storedUsername) {
      setUsername(storedUsername);
      usernameRef.current = storedUsername; // ref bhi update karo
    } else {
      router.push("/joinroom");
    }
  }, [router]);
  ...
  useEffect(() => {
    ...
    stomp.connect({}, () => {
      ...
      // ── Room messages ──
      roomSubscription = stomp.subscribe(
        `/topic/room/${roomId}`,
        (frame) => {
          const newMessage: Message = JSON.parse(frame.body);
          setMessages((prev) => [...prev, newMessage]);

          // KEY FIX: naya message aaya — agar doosre ka hai to turant SEEN bhejo
          // (kyunki hum already is room mein hain)
          if (newMessage.sender !== usernameRef.current) {
            stomp.send(
              `/app/seen/${roomId}`,
              {},
              JSON.stringify({ viewer: usernameRef.current })
            );
          }
        }
      );
      ...
      // ── SEEN events ──
      seenSubscriptionRef.current = stomp.subscribe(
        `/topic/seen/${roomId}`,
        () => {
          // Apne bheje sare messages SEEN mark karo
          setMessages((prev) =>
            prev.map((msg) =>
      ...
      // Room open hote hi SEEN bhejo — purane unread messages ke liye
      stomp.send(
        `/app/seen/${roomId}`,
        {},
        JSON.stringify({ viewer: username })
      );
    });
    ...
    >
            ...
            return (
              <div key={i} className={`flex ${isOwn ? "justify-end" : "justify-start"}`}>
                ...
                <div className={`max-w-[72%] ${isOwn ? "items-end" : "items-start"} flex flex-col gap-1`}>
                  {!isOwn && (
                    <span className="text-xs text-white/40 pl-1">{msg.sender}</span>
                  )}
                  ...
                  >
                    {msg.content && (
                      <p className="break-words leading-relaxed">
                        {isUrl(msg.content) ? (
                          <a href={msg.content} target="_blank" rel="noopener noreferrer" className="text-yellow-300 underline underline-offset-2">
                            {msg.content}
                          </a>
                        ) : msg.content}
                      </p>
                    ...
                    {ft(msg) === "image" && msg.fileUrl && (
                      <img src={msg.fileUrl} alt="shared" className="mt-2 rounded-xl max-w-full max-h-64 object-cover" />
                    )}
                    ...
                    {ft(msg) === "file" && msg.fileUrl && (
                      <a href={msg.fileUrl} target="_blank" rel="noopener noreferrer" className="mt-2 flex items-center gap-2 text-yellow-300 underline underline-offset-2 text-sm">
                        📄 Download File
                      </a>
                    )}
                  </div>
      ...
      {showCamera && (
        <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-sm flex flex-col items-center justify-center gap-5">
          ...
          <div className="flex gap-4">
            <button onClick={capturePhoto} className="px-6 py-2.5 rounded-full bg-indigo-500 hover:bg-indigo-600 text-white font-medium transition">
              📸 Capture
            </button>
            <button onClick={stopCamera} className="px-6 py-2.5 rounded-full bg-red-500/80 hover:bg-red-600 text-white font-medium transition">
              <IoClose className="inline mr-1" /> Close
            </button>
          </div>
        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-30 px-4 pb-4 pt-2">
        <div className="max-w-3xl mx-auto">
          ...
          {selectedFile && (
            <div className="mb-2 flex items-center gap-2 px-4 py-2 rounded-xl bg-white/10 border border-white/10 backdrop-blur-sm text-sm text-white/70">
              <span className="truncate flex-1">📎 {selectedFile.name}</span>
              <button onClick={() => setSelectedFile(null)} className="text-white/40 hover:text-white transition">
                <IoClose size={16} />
              </button>
            </div>
          )}
          ...
          <div className="flex items-center gap-2 bg-white/8 backdrop-blur-xl border border-white/15 rounded-2xl px-3 py-2.5 shadow-2xl">
            ...
            <button
              type="button"
              onClick={isRecording ? stopRecording : startRecording}
              className={`p-2.5 rounded-xl transition flex-shrink-0 ${isRecording ? "text-red-400 bg-red-500/20 animate-pulse" : "text-white/50 hover:text-white hover:bg-white/10"}`}
              title={isRecording ? "Stop recording" : "Voice message"}
            >
              {isRecording ? <FaStop size={15} /> : <FaMicrophone size={16} />}
            </button>
